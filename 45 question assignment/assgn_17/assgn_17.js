...
guests.push("Mike");
// print the message that new table is not arriving in time
console.log("Sorry, the new table is not arriving in time");
// remove all guests until there are only 2 left, and applogize to them
while (guests.length > 2) {
    console.log(`Sorry ${guests.pop()}, I can't invite you to dinner`);
}
// inform the remaining all guests that they are still invited
for (let i = 0; i < guests.length; i++) {
    console.log(`You are still invited to dinner ${guests[i]}`);
}
// remove all guests from the array
guests.splice(0, guests.length);
// print the message that there are no guests left
console.log("There are no guests left");